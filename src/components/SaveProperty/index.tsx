...
export function SaveProperty({ newPropertyToSave }: SavePropertyProps) {
  ...
  const savedProperties: Property[] = savedPropertiesStr ? JSON.parse(savedPropertiesStr) : []

  const isPropertySaved = savedProperties.find((property) => (property.Id === newPropertyToSave?.Id))
      ...
      localStorage.setItem(
        ...
        JSON.stringify(
          savedProperties.filter((property) => (
            property.Id !== newPropertyToSave?.Id
          ))
        )
      )
    } else {
  ...
  return (
    <>
      <Button.Primary className="flex gap-2 items-center ml-auto font-bold text-sm" onClick={handleSavePropertyButtonClick}>
        {isPropertySaved ? <Heart size={16} fill="red" stroke="red" /> : <Heart size={16} />}
        <span>
          {isPropertySaved ? "Saved Property" : "Save Property"}
        </span>
      </Button.Primary>

      <Modal title="Saved Properties" isOpen={isModalOpen} onClose={handleModalClose}>
